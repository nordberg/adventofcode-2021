...
pub fn power_consumption() -> io::Result<()> {
    ...
    for line in reader.lines() {
        ...
        foo.enumerate().for_each(|(i, c)| {
            if c == '1' {
                bit_histogram
                    .entry(i)
                    .and_modify(|b: &mut Bits| b.ones += 1)
                    .or_insert(Bits { ones: 1, zeros: 0 });
            } else {
                bit_histogram
                    .entry(i)
                    .and_modify(|b: &mut Bits| b.zeros += 1)
                    .or_insert(Bits { ones: 0, zeros: 1 });
            }
        });
    }
    ...
    println!(
        "Power rate: {:?}",
        isize::from_str_radix(&*power_rate, 2).unwrap()
    ); // 1143
    println!(
        "Gamma rate: {:?}",
...
fn calculate_generator_rating(bit_strs: Vec<&str>, idx: usize) -> isize {
    ...
    let mut counter = Bits { ones: 0, zeros: 0 };
    ...
    calculate_generator_rating(
        bit_strs
            .iter()
            .filter(|bit_str| bit_str.chars().nth(idx).unwrap() == most_common_bit)
            .map(|s| *s)
            .collect(),
        idx + 1,
    )
